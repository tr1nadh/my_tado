<script>
	...
	$: selectedDate = dateOptions.some((option) => option.value === selectedDate) ? selectedDate : formatDateValue(initialUpcomingDate);
	...
	$: openWeekTasks = searchedTasks.filter((task) => !task.done);
